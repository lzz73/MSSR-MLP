...
macs, params = get_model_complexity_info(model_restoration, (6,512,512), as_strings=True,print_per_layer_stat = False, verbose=True)
...
window_blurL = Visdom(port=opt['PORT'])
window_blurR = Visdom(port=opt['PORT'])
...
for epoch in range(start_epoch, opt['TRAIN']['TOTAL_EPOCHS'] + 1):
    ...
    for iter, data in enumerate(train_loader, 1):
        ...
        if iter % opt['TRAIN']['PRINT_FRE'] == 0:
            log("Epoch:%-4d \tIter:%-4d \tTime:%-4.f \tL1_loss:%.6f \tFFT_loss:%.6f"%(epoch, iter, time.time()-epoch_start_time, loss_L1*100, loss_fft),train_log)
            
            step = epoch + iter/total_iters
            window_loss.line([[loss_L1.item()*100,loss_fft.item()]], [step], win = 'train_loss', update='append')
            window_blurL.images(input_[0,:3,:,:], nrow=1, win='BlurL_Train', opts=dict(title='BlurL'))
            window_blurR.images(input_[0,3:6,:,:], nrow=1, win='BlurR_Train', opts=dict(title='BlurR'))
    ...
    if epoch >= opt['TRAIN']['VAL']:
        if epoch % opt['TRAIN']['VAL_FRE'] == 0:
            ...
            for ii, data_val in enumerate(tqdm(val_loader), 1):
                ...
                psnr_val_rgb.append(PSNR(target, restored, data_range=1))
