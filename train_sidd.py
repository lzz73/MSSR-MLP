...
mixup = utils.dataset_RGB.MixUp_AUG()
######### Model ###########
...
macs, params = get_model_complexity_info(model_restoration, (3,128,128), as_strings=True,print_per_layer_stat = False, verbose=True)
...
log('trainset length: %d \ttotal iters per epoch: %d \t'%(trainset_len, total_iters), train_log)
...
for epoch in range(start_epoch, opt['TRAIN']['TOTAL_EPOCHS'] + 1):
    ...
    for iter, data in enumerate(train_loader, 1):
        ...
        if iter % opt['TRAIN']['PRINT_FRE'] == 0:
            log("Epoch:%-4d \tIter:%-4d \tTime:%-4.f \tL1_loss:%.6f \tFFT_loss:%.6f \t"%(epoch, iter, time.time()-epoch_start_time, loss_L1, loss_fft),train_log)
            
            step = epoch + iter/total_iters
            window_loss.line([[loss_L1.item(),loss_fft.item()]], [step], win = 'train_loss', update='append')
            window_blur.images(input_[0], nrow=1, win='Blur_Train', opts=dict(title='Noise'))
    ...
    if epoch >= opt['TRAIN']['VAL']:
        if epoch % opt['TRAIN']['VAL_FRE'] == 0:
            log('eval epoch %d images'%(epoch), train_log)
